...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold">Categorias de Documentos</h1>
    </div>
    ...
    <div class="mb-6 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div class="px-6 py-4">
            ...
            <form method="GET" action="{{ route('documento-categorias.index') }}" class="flex gap-3 items-end flex-wrap">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('documento-categorias.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 text-gray-700 px-4 py-2 text-theme-sm font-medium hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                        Limpar
                    </a>
                @endif
            </form>
                    ...
                    <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]" data-row-id="{{ $categoria->id }}">
                        ...
                        <td class="px-6 py-3.5">
                            <div class="flex items-center gap-2">
                                ...
                                <form action="{{ route('documento-categorias.destroy', $categoria) }}" method="POST" class="inline" @submit.prevent="deletarCategoria($event, {{ $categoria->id }})">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-12 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm font-medium">Nenhuma categoria encontrada</p>
                                <button onclick="openCreateCategoriaModal()" class="mt-4 inline-flex items-center gap-2 rounded-lg bg-green-600 text-white px-4 py-2 text-theme-sm font-medium hover:bg-green-700">
        ...
        @forelse($categorias as $categoria)
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
                ...
                <div class="flex gap-2 mt-3">
                    ...
                    <form action="{{ route('documento-categorias.destroy', $categoria) }}" method="POST" class="flex-1" @submit.prevent="deletarCategoria($event, {{ $categoria->id }})">
                        @csrf @method('DELETE')
        ...
        @empty
            <div class="py-12 text-center text-gray-500 dark:text-gray-400">
                <div class="flex flex-col items-center justify-center">
                    ...
                    <p class="text-sm font-medium mb-4">Nenhuma categoria encontrada</p>
                    <button onclick="openCreateCategoriaModal()" class="inline-flex items-center gap-2 rounded-lg bg-green-600 text-white px-4 py-2 text-theme-sm font-medium hover:bg-green-700">
                        <svg class="fill-current" width="16" height="16" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 5v14m7-7H5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                        Nova Categoria
...
<script>
    ...
    async function submitCategoriaForm(event) {
        ...
        const url = id ? `/documento-categorias/${id}` : '/documento-categorias';
    ...
    function deletarCategoria(event, categoriaId) {
        event.preventDefault();

        Swal.fire({
            title: 'Confirmar exclusão',
            text: 'Tem certeza que deseja deletar esta categoria? Esta ação não pode ser desfeita.',
            icon: 'warning',
