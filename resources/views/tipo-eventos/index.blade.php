...
<div x-data="tiposEventoManager()" class="container mx-auto px-4 py-8">
    ...
    <div class="mb-6 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div class="px-6 py-4">
            ...
            <form method="GET" action="{{ route('tipo-eventos.index') }}" class="flex gap-3 items-end flex-wrap">
                ...
                @if(request('search'))
                    <a href="{{ route('tipo-eventos.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 text-gray-700 px-4 py-2 text-theme-sm font-medium hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                        Limpar
                    </a>
                @endif
            </form>
        ...
        @forelse($tiposEvento as $tipoEvento)
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
                <div class="mb-3 flex items-start justify-between">
                    <div class="flex-1">
                        <h4 class="text-base font-medium text-gray-800 dark:text-white/90">{{ $tipoEvento->nome }}</h4>
                        <p class="text-theme-xs text-gray-500 dark:text-gray-400 mt-1">{{ substr($tipoEvento->descricao ?? '-', 0, 60) }}</p>
                    </div>
                </div>
                ...
                <div class="flex gap-2 mt-3">
                    ...
                    <form action="{{ route('tipo-eventos.destroy', $tipoEvento) }}" method="POST" class="flex-1"
                          @submit.prevent="deleteItem({{ $tipoEvento->id }}, $event)">
                        @csrf
                        @method('DELETE')
