...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600 dark:text-gray-400">Atualizar fornecedor para {{ $evento->nome }}</p>
    </div>

    <!-- Form Card -->
    <div class="rounded-2xl border border-gray-200 bg-white p-8 dark:border-white/[0.05] dark:bg-white/[0.03] max-w-2xl">
        <form action="{{ route('evento-tipos-camiseta.update', [$evento, $tipo]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                >
                    ...
                    @forelse ($fornecedores as $fornecedor)
                        <option value="{{ $fornecedor->id }}" @selected(old('fornecedor_id', $tipo->fornecedor_id) == $fornecedor->id)>
                            {{ $fornecedor->nome }}
                        </option>
                    @empty
