...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-3xl font-bold mb-6">Criar Nova Secretaria</h1>

    <form action="{{ route('secretarias.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf
        ...
        <div>
            ...
            <select name="entidade_pai_id" class="w-full px-4 py-2 border rounded-lg @error('entidade_pai_id') border-red-500 @enderror" required>
                ...
                @foreach ($nucleos as $nucleo)
                    <option value="{{ $nucleo->id }}" @selected(old('entidade_pai_id') == $nucleo->id)>{{ $nucleo->nome }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="tipo_secretaria" class="w-full px-4 py-2 border rounded-lg @error('tipo_secretaria') border-red-500 @enderror" required>
                ...
                <option value="aberta" @selected(old('tipo_secretaria') == 'aberta')>Aberta</option>
                <option value="fechada" @selected(old('tipo_secretaria') == 'fechada')>Fechada</option>
            </select>
        ...
        <div class="pt-4">
            ...
            <a href="{{ route('secretarias.index') }}" class="ml-2 px-6 py-2 text-gray-600 hover:text-gray-800">
                Cancelar
            </a>
        </div>
    </form>
