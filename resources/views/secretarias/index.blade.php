    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 px-6 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-white/[0.05]">
            ...
            <div class="flex items-center gap-3">
                ...
                <button onclick="openModal('secretariaModal', false)"
                ...
                <tbody>
                    @forelse($secretarias as $secretaria)
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]" data-row-id="{{ $secretaria->id }}">
                            ...
                            <td class="px-6 py-3.5">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ $secretaria->entidadePai->nome ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-3.5">
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <button onclick="openModal('secretariaModal', true, {
                                        id: {{ $secretaria->id }},
                                        entidade_pai_id: {{ $secretaria->entidade_pai_id }},
                                        nome: '{{ addslashes($secretaria->nome) }}',
                                        email: '{{ addslashes($secretaria->email ?? '') }}',
                                        tipo_secretaria: '{{ $secretaria->tipo_secretaria }}',
                                        ativo: {{ $secretaria->ativo ? 'true' : 'false' }}
                                    })"
    ...
    >
        <div>
            ...
            <select
                ...
                id="secretariaModalentidade_pai_id"
            ...
            <span class="text-red-500 text-sm" id="secretariaModalentidade_pai_idError"></span>
        </div>
    ...
    <script>
        function secretariasManager() {
            return {
                ...
                deleteSelected() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            fetch('/secretarias/delete-multiple', {
                                method: 'POST',
                                body: formData,
                                headers: {
                              ...
                              .then(data => {
                                  if (data.success) {
                                      Swal.fire({
                                          icon: 'success',
                                          title: 'Sucesso!',
                                          text: 'Secretarias deletadas com sucesso!',
                ...
                deleteItem(secretariaId, event) {
                    event.preventDefault();
                    Swal.fire({
                        icon: 'warning',
                        title: 'Confirmar exclusão',
                        text: 'Tem certeza que deseja deletar esta secretaria?',
                        showCancelButton: true,
                        confirmButtonColor: '#d33',
                        confirmButtonText: 'Deletar',
                        cancelButtonText: 'Cancelar'
