...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Editar Tamanho</h1>
        <p class="text-gray-600 dark:text-gray-400">Atualizar tamanho {{ $tamanho->tamanho }}</p>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white p-8 dark:border-white/[0.05] dark:bg-white/[0.03] max-w-2xl">
        <form action="{{ route('fornecedores-camisetas.tipos.tamanhos.update', [$fornecedor, $tipo, $tamanho]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input
                    type="text"
                    name="tamanho"
                    value="{{ old('tamanho', $tamanho->tamanho) }}"
            ...
            <div class="mb-6">
                ...
                >{{ old('medidas', is_array($tamanho->medidas) ? json_encode($tamanho->medidas) : $tamanho->medidas) }}</textarea>
                @error('medidas')
                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-8">
                <label class="flex items-center gap-3 dark:text-gray-300">
                    <input
                        ...
                        name="ativo"
                        value="1"
                        @checked(old('ativo', $tamanho->ativo))
