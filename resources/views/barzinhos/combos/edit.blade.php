...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Editar Combo</h1>
        <p class="text-gray-600 dark:text-gray-400">Atualizar {{ $combo->nome }}</p>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white p-8 dark:border-white/[0.05] dark:bg-white/[0.03] max-w-2xl">
        <form action="{{ route('barzinhos.combos.update', [$barzinho, $combo]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input
                    ...
                    placeholder="Ex: Combo Refrigerante + Cerveja"
                    required
                >
                @error('nome')
            ...
            <div class="mb-6">
                ...
                <textarea
                    ...
                    placeholder="Descrição dos itens do combo"
                >{{ old('descricao', $combo->descricao) }}</textarea>
                @error('descricao')
                    <p class="mt-1 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input
                    type="number"
                    name="preco"
                    value="{{ old('preco', $combo->preco) }}"
                    step="0.01"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('preco') border-red-500 @enderror"
                    placeholder="0.00"
                    required
                >
                @error('preco')
            ...
            <div class="flex gap-3 pt-6 border-t border-gray-200 dark:border-white/[0.05]">
                ...
                <a
                    href="{{ route('barzinhos.combos.index', $barzinho) }}"
