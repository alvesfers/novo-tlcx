...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold">Editar Barzinho</h1>
        <a href="{{ route('barzinhos.show', $barzinho) }}" class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400">
            Voltar
        </a>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('barzinhos.update', $barzinho) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                >
                    ...
                    @foreach(\App\Models\Evento::ativos()->get() as $evento)
                        <option value="{{ $evento->id }}" {{ old('evento_id', $barzinho->evento_id) == $evento->id ? 'selected' : '' }}>
                            {{ $evento->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                >{{ old('descricao', $barzinho->descricao) }}</textarea>
                @error('descricao')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="flex items-center gap-2">
                    <input
                        ...
                        name="ativo"
                        value="1"
                        {{ old('ativo', $barzinho->ativo) ? 'checked' : '' }}
                        class="rounded"
                    >
            ...
            <div class="flex gap-4 pt-4">
                ...
                <a
                    href="{{ route('barzinhos.show', $barzinho) }}"
