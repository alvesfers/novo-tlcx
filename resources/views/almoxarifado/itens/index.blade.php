...
<div class="container mx-auto px-4 py-8">
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 px-6 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-white/[0.05]">
            ...
            <button onclick="openModal('itemModal')"
                    ...
                    <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                        ...
                        <td class="px-6 py-3.5">
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold"
                                @if($item->status->value === 'ativo') class="bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200"
                                @elseif($item->status->value === 'esgotado') class="bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200"
                                @else class="bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200" @endif>
                                {{ $item->status->label() }}
                            </span>
                        </td>
                        <td class="px-6 py-3.5">
                            <div class="flex items-center gap-3">
                                <button onclick="openModal('itemModal', true, {
                                    id: {{ $item->id }},
                                    nome: '{{ addslashes($item->nome) }}',
                                    almoxarifado_categoria_id: {{ $item->almoxarifado_categoria_id }},
                                ...
                                <form action="{{ route('almoxarifado-itens.destroy', $item) }}" method="POST" style="display: inline;">
                                    @csrf @method('DELETE')
...
>
    <div class="space-y-4">
        ...
        <div>
            ...
            <select name="almoxarifado_categoria_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Unidade de Medida</label>
            <select name="unidade_medida" class="w-full px-4 py-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="quantidade_minima" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600">
            </div>
            <div>
