...
<div x-data="diocesesManager()" class="container mx-auto px-4 py-8" id="diocesesContainer">
    ...
    <div class="mb-6 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div class="px-6 py-4">
            ...
            <form method="GET" action="{{ route('dioceses.index') }}" class="flex gap-3 items-end flex-wrap">
                ...
                @if($selectedStatus || $searchQuery)
                    <a href="{{ route('dioceses.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-gray-200 text-gray-700 px-4 py-2 text-theme-sm font-medium hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                        Limpar
                    </a>
                @endif
            </form>
        ...
        <div class="flex flex-col gap-4 px-6 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-white/[0.05]">
            ...
            <div class="flex items-center gap-3">
                ...
                @can('create', App\Models\Entidade::class)
                    <button onclick="openCreateDioceseModal()"
            ...
            <table class="w-full">
                ...
                <tbody>
                    @forelse($dioceses as $diocese)
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]" data-row-id="{{ $diocese->id }}">
                            @can('deleteMultiple')
                                <td class="px-6 py-3.5">
                                    <div class="flex items-center gap-3">
                                        <div @click="handleRowSelect({{ $diocese->id }})"
                                            class="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border-[1.25px]"
                                            :class="selectedRows.includes({{ $diocese->id }}) ? 'border-blue-500 dark:border-blue-500 bg-blue-500' : 'bg-white dark:bg-white/0 border-gray-300 dark:border-gray-700'">
                                            <svg :class="selectedRows.includes({{ $diocese->id }}) ? 'block' : 'hidden'" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M11.6668 3.5L5.25016 9.91667L2.3335 7" stroke="white" stroke-width="1.94437" stroke-linecap="round" stroke-linejoin="round"/>
                                            </svg>
                                        </div>
                                    </div>
                            ...
                            @endcan
                            <td class="px-6 py-3.5">
                                @if($diocese->foto_arquivo || $diocese->foto_url)
                                    <img
                                ...
                                @else
                                    <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 text-sm font-bold">
                                        {{ substr($diocese->nome, 0, 1) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-3.5">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ $diocese->nome }}</p>
                            </td>
                            <td class="px-6 py-3.5">
                                <p class="text-gray-700 text-theme-sm dark:text-gray-400">{{ $diocese->email ?? '-' }}</p>
                            </td>
                            <td class="px-6 py-3.5">
                                <span class="inline-block px-3 py-1 rounded-full text-theme-xs font-medium
                                    ...
                                    @endif">
                                    {{ $diocese->ativo ? 'Ativo/a' : 'Inativo/a' }}
                                </span>
                            </td>
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('dioceses.show', $diocese) }}"
                                    ...
                                    <button onclick="openInfoModal('diocese', {{ $diocese->id }}, '{{ $diocese->nome }}')"
                                    ...
                                    @can('update', $diocese)
                                        <button onclick="openEditDioceseModal({{ $diocese->id }}, '{{ addslashes($diocese->nome) }}', '{{ addslashes($diocese->email ?? '') }}', {{ $diocese->ativo ? '1' : '0' }})"
                                    ...
                                    @can('delete', $diocese)
                                        <form action="{{ route('dioceses.destroy', $diocese) }}" method="POST" class="inline"
                                              @submit.prevent="deleteItem({{ $diocese->id }}, $event)">
                                            @csrf
                                            @method('DELETE')
        ...
        @forelse($dioceses as $diocese)
            <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/[0.05] dark:bg-white/[0.03]">
                <div class="mb-3 flex items-start justify-between">
                    <div class="flex-1">
                        <h4 class="text-base font-medium text-gray-800 dark:text-white/90">{{ $diocese->nome }}</h4>
                        <p class="text-theme-xs text-gray-500 dark:text-gray-400 mt-1">{{ $diocese->email ?? 'sem email' }}</p>
                    </div>
                ...
                <div class="flex gap-2 mt-3">
                    ...
                    @can('delete', $diocese)
                        <form action="{{ route('dioceses.destroy', $diocese) }}" method="POST" class="flex-1"
                              @submit.prevent="deleteItem({{ $diocese->id }}, $event)">
                            @csrf
                            @method('DELETE')
...
<script>
    ...
    function openEditDioceseModal(id, nome, email, ativo) {
        const titleEl = document.getElementById('dioceseModalTitle');
        const submitBtn = document.getElementById('dioceseSubmitBtn');
        const idEl = document.getElementById('dioceseId');
        const nameEl = document.getElementById('dioceseName');
        const emailEl = document.getElementById('dioceseEmail');
        const ativoEl = document.getElementById('dioceseAtivo');
        const ativoField = document.getElementById('ativoField');

        if (titleEl) titleEl.textContent = 'Editar Diocese';
    ...
    async function submitDioceseForm(event) {
        ...
        const url = id ? `/dioceses/${id}` : '/dioceses';
        const method = id ? 'PUT' : 'POST';

        const formData = new FormData(document.getElementById('dioceseModalForm'));
        ...
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Sucesso!',
                text: id ? 'Diocese atualizada com sucesso!' : 'Diocese criada com sucesso!',
