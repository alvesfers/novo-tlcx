...
@section('content')
<div x-data="dirigentesManager()" class="container mx-auto px-4 py-8">
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 px-6 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-100 dark:border-white/[0.05]">
            ...
            <div class="flex items-center gap-3">
                ...
                <button onclick="openModal('dirigenteModal', false)"
                    ...
                    @forelse($dirigentes as $dirigente)
                        <tr class="border-b border-gray-100 dark:border-white/[0.05] hover:bg-gray-50 dark:hover:bg-white/[0.02]" data-row-id="{{ $dirigente->id }}">
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div @click="handleRowSelect({{ $dirigente->id }})"
                                        class="flex h-5 w-5 cursor-pointer items-center justify-center rounded-md border-[1.25px]"
                                        :class="selectedRows.includes({{ $dirigente->id }}) ? 'border-blue-500 dark:border-blue-500 bg-blue-500' : 'bg-white dark:bg-white/0 border-gray-300 dark:border-gray-700'">
                                        <svg :class="selectedRows.includes({{ $dirigente->id }}) ? 'block' : 'hidden'" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M11.6668 3.5L5.25016 9.91667L2.3335 7" stroke="white" stroke-width="1.94437" stroke-linecap="round" stroke-linejoin="round"/>
                                        </svg>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <button onclick="openModal('dirigenteModal', true, {
                                        id: {{ $dirigente->id }},
                                        ...
                                        entidade_id: {{ $dirigente->vinculos->first()?->entidade_id ?? 0 }},
                                    ...
                                    <form action="{{ route('dirigentes.destroy', $dirigente) }}" method="POST" class="inline"
                                          @submit.prevent="deleteItem({{ $dirigente->id }}, $event)">
                                        @csrf
                                        @method('DELETE')
    ...
    >
        <div>
            ...
            <input
                type="text"
                name="nome"
                id="dirigenteModalnome"
                class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                required
            >
            <span class="text-red-500 text-sm" id="dirigenteModalnomeError"></span>
        </div>
        ...
        <div>
            ...
            <input
                type="text"
                name="telefone"
                id="dirigenteModaltelefone"
                class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
            <span class="text-red-500 text-sm" id="dirigenteModaltelefoneError"></span>
        </div>
        ...
        <div>
            ...
            <span class="text-red-500 text-sm" id="dirigenteModalgeneroError"></span>
        </div>
        ...
        <div>
            ...
            <input
                ...
                id="dirigenteModaldata_nascimento"
                class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
            <span class="text-red-500 text-sm" id="dirigenteModaldata_nascimentoError"></span>
        </div>

        <div>
            <label class="block text-sm font-medium mb-2 dark:text-gray-200">Foto URL</label>
            <input
                type="url"
                name="foto_url"
                id="dirigenteModalfoto_url"
                class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            >
            <span class="text-red-500 text-sm" id="dirigenteModalfoto_urlError"></span>
        </div>

        <div>
            <label class="block text-sm font-medium mb-2 dark:text-gray-200">Núcleo Principal</label>
            <select
                name="entidade_id"
                id="dirigenteModalentidade_id"
                class="w-full px-4 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                required
            >
                <option value="">Selecione um núcleo...</option>
                @foreach($nucleos as $nucleo)
                    <option value="{{ $nucleo->id }}">{{ $nucleo->nome }} ({{ $nucleo->entidadePai?->nome }})</option>
                @endforeach
            </select>
            <span class="text-red-500 text-sm" id="dirigenteModalentidade_idError"></span>
        </div>
        ...
        <div data-edit-only style="display: none;">
            ...
            <label class="flex items-center dark:text-gray-200">
                <input type="checkbox" name="ativo" id="dirigenteModalativo" value="1" class="rounded dark:bg-gray-700">
    ...
    <script>
        function dirigentesManager() {
            return {
                ...
                deleteSelected() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            fetch('/dirigentes/delete-multiple', {
                                method: 'POST',
                                body: formData,
                                headers: {
                              ...
                              .then(data => {
                                  if (data.success) {
                                      Swal.fire({
                                          icon: 'success',
                                          title: 'Sucesso!',
                                          text: 'Dirigentes deletados com sucesso!',
                ...
                deleteItem(dirigenteId, event) {
                    event.preventDefault();
                    Swal.fire({
                        icon: 'warning',
                        title: 'Confirmar exclusão',
                        text: 'Tem certeza que deseja deletar este dirigente?',
                        showCancelButton: true,
                        confirmButtonColor: '#d33',
                        confirmButtonText: 'Deletar',
                        cancelButtonText: 'Cancelar'
